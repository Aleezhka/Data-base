...
def delete_ticket(ticket_id: int) -> Response:
    ...
    #     return make_response(jsonify(ticket_controller.get_tickets_by_user_id(user_id)), HTTPStatus.OK)
...
def get_tickets_by_user(user_id: int) -> Response:
    ...
    if user:
        tickets = user.tickets  # Отримуємо всі квитки, що належать цьому користувачу
...
def get_users_by_ticket(ticket_id: int) -> Response:
    ...
    if ticket:
        users = ticket.users  # Отримуємо всіх користувачів, що мають цей квиток
